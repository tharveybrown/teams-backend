...
class SessionsController < ApplicationController
  ...
  def create
    ...
    # response = client.oauth_access({client_id: ENV['SLACK_CLIENT_ID'], client_secret: ENV['SLACK_CLIENT_SECRET'], code: temp_code, redirect_uri: 'http://localhost:3001/auth/callback'})
    # byebug
    # team_id = response['team_id']
    uri = URI('https://slack.com/api/oauth.v2.access')
    params = { :client_id => ENV['SLACK_CLIENT_ID'], :client_secret => ENV['SLACK_CLIENT_SECRET'], code: temp_code, redirect_uri: "http://localhost:3001/auth/callback" }
    ...
    slack_team = SlackTeam.find_or_create_by(slack_id: response['team']['id'], name: response['team']['name'], organization_id: Organization.last.id)
